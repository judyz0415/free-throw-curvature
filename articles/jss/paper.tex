...
\bibliographystyle{\ifnum\pdfstrcmp{\jobname}{output}=0 articles/apa-good\else ../apa-good\fi}
  ...
  \section{Methods}
    ...
    To quantify shot smoothness and examine its relationship with free throw accuracy, this section introduces a curvature-based metric that captures the degree of bending in the ball's trajectory. Following \citet{slegers_role_2024}, we calculate curvature in the X-Z plane, ignoring lateral movement of the ball during the shooting motion. Bézier curves are particularly advantageous for this purpose due to their smoothness, continuity and differentiability, which allow for precise curvature computation. 
        ...
        \subsubsection{Bézier Curves}
            ...
            Figure \ref{fig:free-throw-bezier} shows an example of a free throw shot modeled using an eighth-order Bézier curve (i.e.\ nine control points). As seen in the figure, the curve closely follows the observed data, capturing both the smooth upward arc and the transition into the release phase of the shot.
            ...
            To select $n$ in a principled manner, we fit Bézier curves of orders $n = 3, 4, \ldots, 12$ to every shot in the dataset and compute the mean in-sample mean squared fitting error (MSE) across all shots for each candidate order. As $n$ increases, the MSE decreases monotonically---additional control points allow the curve to conform more closely to the observed trajectory---but with diminishing marginal returns. We identify the optimal order as the ``knee'' of the MSE-versus-$n$ curve: the order beyond which the marginal reduction in fitting error becomes proportionally negligible. Formally, letting $\Delta_n = \overline{\text{MSE}}_{n-1} - \overline{\text{MSE}}_n$ denote the marginal reduction at step $n$, we select the order minimising the ratio $\Delta_{n+1} / \Delta_n$, i.e.\ the point at which the marginal return drops most sharply relative to the previous step. Figure~\ref{fig:bezier-order} shows the mean fitting MSE and the marginal reductions across candidate orders; this procedure selects $n = 8$, which we adopt throughout this study. This is comparable to the order $n = 6$ used by \citet{slegers_role_2024}.

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.8\linewidth]{select_bezier_order.png}
                \caption{\it Selection of the Bézier curve order. Left: mean in-sample fitting MSE across all shots as a function of order $n$; right: marginal MSE reduction $\Delta_n$ from order $n-1$ to $n$. The red dashed line marks the selected order $n = 8$, identified as the knee of the error curve where the ratio of consecutive marginal reductions is minimised.}
                \label{fig:bezier-order}
            \end{figure}
...
%          \citet{slegers_role_2024} found that the max curvature $\kappa_\max = \max_\tau [\kappa(\tau)]$ during the ball path negatively correlates with players' free throw shooting accuracy. The max curvature generally occurs at the transition between the players' backward swing and the forward motion of ball release, where a high max curvature indicates a more distinctive two-part motion of the shot. Max curvature could be easily extracted by taking the maximum of the instantaneous curvature values along the ball paths as computed above. 
        ...
        \subsubsection{Selection of the Weighting Parameter}
        \label{sec:l-selection}

          The weighting parameter $\ell$ governs how strongly $\sigma(\ell)$ emphasises curvature near the end of the ball path, and its optimal value is unknown a priori. Rather than fixing $\ell$ arbitrarily, we select it using leave-one-out cross-validation (LOO-CV) at the player level, evaluating prediction accuracy across all half-integer candidates $\ell \in \{0,\, 0.5,\, 1.0,\, \ldots,\, 15.0\}$.

          For each candidate $\ell$ and each held-out player $p$, we fit a weighted least squares (WLS) regression of FT\% onto $\sigma(\ell)$ on the remaining $n - 1 = 34$ players and record the squared prediction error for the held-out player, $({\widehat{\text{FT\%}}}_p - \text{FT\%}_p)^2$. Each player's observation is weighted by $1 / (\text{FT\% SE}_p)^2$, where $\text{FT\% SE}_p = \sqrt{\widehat{\text{FT\%}}_p (1 - \widehat{\text{FT\%}}_p) / \text{FTA}_p}$ is the estimated standard error of the player's FT\%. This is consistent with the weighting scheme in model (\ref{eqn:weighted-least-squares}). We then average these squared errors across all $n = 35$ players to obtain the LOO-CV mean squared prediction error (MSE) for $\ell$. The optimal weighting parameter is
          \[
            \ell^* = \arg\min_{\ell} \frac{1}{n}\sum_{p=1}^{n}\bigl(\widehat{\text{FT\%}}_p^{(-p)} - \text{FT\%}_p\bigr)^2,
          \]
          where $\widehat{\text{FT\%}}_p^{(-p)}$ denotes the prediction for player $p$ from the model fitted without that player.

          Figure~\ref{fig:l-selection} displays the LOO-CV MSE and in-sample $R^2$ as functions of $\ell$. The LOO-CV MSE is minimised at $\ell = 6$, which we therefore adopt as our primary weighting parameter. The in-sample $R^2$ rises steeply as $\ell$ increases from 0 and plateaus around $\ell = 3$--$6$, consistent with the interpretation that curvature near the end of the shot is the most predictive portion of the trajectory, and that gains from further increasing $\ell$ beyond this range are negligible.

          \begin{figure}[H]
              \centering
              \includegraphics[width=0.8\linewidth]{select_l_parameter.png}
              \caption{\it Selection of the weighting parameter $\ell$ via player-level leave-one-out cross-validation (LOO-CV). Left: mean LOO squared prediction error for FT\% as a function of $\ell$; right: corresponding in-sample $R^2$ from the WLS regression of FT\% onto $\sigma(\ell)$. The red dashed line marks the CV-selected optimum at $\ell = 6$.}
              \label{fig:l-selection}
          \end{figure}
    ...
    \subsection{Sample size estimation and justification}
    
    This study is an exploratory study as outlined and defined by \citet{ditroilo_exploratory_2025}. The sample size ($n = 35$ players) was determined by data availability rather than through an a priori power calculation. Therefore, a sensitivity power analysis was performed to estimate the effect sizes detectable with the sample used. Given $n = 35$, an analysis conducted in G*Power \citep{faul_statistical_2009} indicated that a two-sided linear regression with $\alpha = 0.05$ and power $= 0.8$ is sufficiently powered to detect a minimum effect size of Cohen’s $f^2 = 0.238$, corresponding to $R^2 = 0.192$. In other words, our analysis is capable of detecting effects where each curvature metric explains at least 19.2\% of the variance in FT\%.
        
\section{Results}
    
    The metrics computed for this study include (a) max curvature $\kappa_{\max} = \max_\tau[\kappa(\tau)]$ and (b) time-weighted curvature integrals with weighting factors $\ell = 0$, $3$, and $6$, where $\ell = 6$ is the CV-selected optimum described in Section~\ref{sec:l-selection}. We also include $\ell = 3$ as an intermediate value to illustrate the effect of progressively increasing the weighting parameter. For each player, these metrics are computed on a per-shot basis and subsequently averaged across all of their recorded free throw attempts (approximately 15 shots per player) to obtain player-level estimates.
    ...
    \begin{table}[H]
        ...
        Time-weighted Curvature Integral ($\ell=6$, CV-selected) & 0.45 & 0.17 & 0.24--0.97 \\ % TODO: update mean/SD/range from result\_analysis output
        \bottomrule
        \end{tabular}
    ...
    Table \ref{tab:wls-results} summarizes the results of estimating the WLS model (\ref{eqn:weighted-least-squares}) for each of the four curvature metrics. Each metric is a statistically significant predictor of FT\%, with each $p$-value below 0.01. Among them, the time-weighted curvature integrals demonstrate the strongest explanatory power, particularly at $\ell = 6$ (the CV-selected optimum), which achieves the highest $R^2$ value among the metrics examined. The confidence bands shown in Figure \ref{fig:regressionlines} illustrate this conclusion: the regression fits at $\ell = 3$ and $\ell = 6$ exhibit relatively narrower and more stable bands, indicating greater precision in the estimated relationship between curvature and FT\% in these models. This finding aligns with the controlled experiments of \citet{slegers_role_2024}, suggesting that lower terminal curvature is correlated with greater shooting performance.
    ...
    \begin{table}[H]
        ...
        Time-weighted Curvature Integral ($\ell=6$, CV-selected) &  --48.73  & 0.0003  & 0.330 \\ % TODO: update $\beta$, $p$-value, and $R^2$ for $\ell=6$ from result\_analysis output
        \bottomrule
        \end{tabular}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\linewidth]{articles/figures/regressionlines.png}
...
\section{Discussion}
    ...
    \subsection{Results}
      ...
      The results of this study provide strong empirical support for the hypothesis that smoother shot trajectories---particularly those with less curvature near the point of release---are associated with greater free throw accuracy in real-game contexts in the NBA. Among the metrics examined, the time-weighted curvature integrals exhibited the strongest predictive power, particularly at the CV-selected weighting parameter $\ell = 6$ (Section~\ref{sec:l-selection}). This aligns with the intuition that late-stage control of the shot is critical for accuracy and supports the data-driven choice of $\ell$.
  ...
  % This nastiness for the bibliography to work both locally and on Overleaf
  % https://tex.stackexchange.com/questions/623701/path-for-bib-file-working-simultaneously-in-texstudio-and-overleaf
  % \bibliography{\ifnum\pdfstrcmp{\jobname}{output}=0 articles/free-throw-curvature\else ../free-throw-curvature\fi}
  \bibliographystyle{apa-good}
  \bibliography{articles/free-throw-curvature}
